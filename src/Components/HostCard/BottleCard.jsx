...
import { useGetAllbottoleQuery } from '../../redux/Api/bottleApi';
import { toast } from 'react-toastify';
import Loading from '../../Loading/Loading';

// Dummy data for design testing - remove when API is ready
const dummyBottleGirls = [
...
const BottleCard = () => {
    const { data, isLoading, isError } = useGetAllbottoleQuery({});
    const [approveHost] = useGetAllaprovedMutation();
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 10;

    // Use API data if available, otherwise use dummy data
    const bottleGirlData = data?.data?.length > 0 ? data.data : dummyBottleGirls;
    ...
    const handleApprove = async (id) => {
        try {
            ...
            toast.success('Bottle Girl approved successfully!');
        } catch (error) {
            toast.error('Error approving Bottle Girl');
        }
    };

    // Pagination logic
    const totalItems = bottleGirlData?.length || 0;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentData = bottleGirlData?.slice(startIndex, endIndex) || [];
    ...
    return (
        <div className="bottle-card-container">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
                {currentData.map((bottleGirl) => {
                    const defaultAvatar = "https://ui-avatars.com/api/?name=" + encodeURIComponent(bottleGirl.name || "User") + "&background=EFC11F&color=020123";
                    const avatarSrc = bottleGirl.avatar ? `${imageUrl}/${bottleGirl.avatar}` : defaultAvatar;
                    ...
                    return (
                        ...
                        >
                            ...
                            />
                            <p className="text-[#000080] font-semibold text-sm text-center mb-1">{bottleGirl.name || "N/A"}</p>
                            <p className="text-gray-500 text-[10px] sm:text-[11px] text-center mb-0.5 break-all">
                                <span className="text-gray-600">Email:</span> {bottleGirl.auth?.email || "N/A"}
                            </p>
                            <p className="text-gray-500 text-[10px] sm:text-[11px] text-center mb-0.5">
                                <span className="text-gray-600">License No:</span> {bottleGirl.licenseNumber || "N/A"}
                            </p>
                            <p className="text-gray-500 text-[10px] sm:text-[11px] text-center mb-3">
                                <span className="text-gray-600">Contact:</span> {bottleGirl.phoneNumber || "N/A"}
                            </p>
                            ...
                            <div className="flex items-center gap-2 mt-auto">
                                ...
                                <button
                                    ...
                                    onClick={() => handleApprove(bottleGirl._id)}
                                >
                                    Approve
                                </button>
                            </div>
