    ...
    <div className="host-card-container">
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4'>
        {currentData.map((host) => {
          const defaultAvatar = "https://ui-avatars.com/api/?name=" + encodeURIComponent(host.name || "User") + "&background=EFC11F&color=020123";
          const avatarSrc = host.avatar ? `${imageUrl}/${host.avatar}` : defaultAvatar;
          ...
          return (
            ...
            >
              ...
              />
              <p className='text-[#000080] font-semibold text-sm text-center mb-1'>{host.name || 'N/A'}</p>
              <p className='text-gray-500 text-[10px] sm:text-[11px] text-center mb-0.5 break-all'>
                <span className='text-gray-600'>Email:</span> {host.auth?.email || 'N/A'}
              </p>
              <p className='text-gray-500 text-[10px] sm:text-[11px] text-center mb-0.5'>
                <span className='text-gray-600'>License No:</span> {host.licenseNumber || 'N/A'}
              </p>
              <p className='text-gray-500 text-[10px] sm:text-[11px] text-center mb-3'>
                <span className='text-gray-600'>Contact:</span> {host.phoneNumber || 'N/A'}
              </p>
