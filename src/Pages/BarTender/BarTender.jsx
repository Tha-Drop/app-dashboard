...
import { useGetAllbartenderApproveQuery } from '../../redux/Api/bartender';
import BartenderCard from '../../Components/HostCard/BartenderCard';
import { imageUrl } from '../../redux/Api/baseApi';

const BarTender = () => {
    const [tab, setTab] = useState(true)
    const [search, setSearch] = useState("")
    const { data } = useGetAllbartenderApproveQuery({});
        ...
        <div className='bg-white p-3 sm:p-4 md:p-6 rounded-lg'>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <span className='font-semibold text-lg sm:text-[20px] text-[#020123]'>Bartender Information</span>
                </div>
            ...
            <div>
                <p className='text-lg my-6 font-semibold text-[#020123]'>
                    {tab ? "All DJ List With Their Request" : "All Bartender Info"}
                </p>

                <div>
                    {tab ? <BartenderCard /> : <GuestHostInfo dataSource={dataSource} />}
                </div>
            </div>
        </div>
    )
